    ...
    try:
        ...
        if shield_config.get("enable"):
            ...
            def shield_logic(cpu, data, size):
                ...
                input_event_data = ("pid:%d" % (event.pid))


                if shield_config["features"]["limit_sendind"]:
                    # implement here your limit_sendind feature (o will crash on run ;)

                    
                message = ""
                try:

                    message = f"{constants.CAETRA_SENDER_LABEL}_{SHIELD_NAME.upper()} act: '{shield_config.get("action_label")}' limit_sendind: {shield_config["features"]["limit_sendind"]} data: { input_event_data }"
