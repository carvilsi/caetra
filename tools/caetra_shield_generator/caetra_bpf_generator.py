...
def write_template_output(shield_name, file_extension, content):
    with open(f"{shield_name}/{shield_name}.{file_extension}", "w", encoding="utf-8") as f:
        f.write(content)
...
def caetra_shield_generator(shield_name, shield_description, kprobe_event, c_function_name, kheaders_include, kstrct, shield_enable, shield_feature, canarytoken_sender, canarytoken, telegram_sender, telegram_chat_id, telegram_bot_api_key):
    ...
    if not os.path.exists(shield_name):
        os.makedirs(shield_name)
    
    env = Environment(loader = FileSystemLoader(FOLDER_TEMPLATE))
    
    # Python Template
    python_template = env.get_template(PYTHON_TEMPLATE)
    python_output = python_template.render(shield_name, kprobe_event, c_function_name, shield_feature)
    write_template_output(shield_name, "py", python_output)    
    
    # C Kernel side Template
    c_template = env.get_template(C_TEMPLATE)
    c_output = c_template.render(shield_name, kheaders_include, kstrct)
    write_template_output(shield_name, "c", c_output)    
    
    # Toml Template
    toml_template = env.get_template(TOML_TEMPLATE)
    toml_output = toml_template.render(shield_name, shield_description, shield_feature, canarytoken_sender, canarytoken, telegram_sender, telegram_chat_id, telegram_bot_api_key)
